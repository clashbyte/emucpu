					...
					if (!Regex.IsMatch(sn, @"^\d{3}$")) {
						...
						if (a == 0 && b == 0 && c == 0) {
							...
							if (a == c) {
								...
								codes.Add(new string[]{
									"mov "+IndexToReg(8)+", "+IndexToReg(a)+"\t\t; Перемещение из "+IndexToReg(8)+" в "+IndexToReg(a)
								});
							} else {
							...
							if (a == b) {
								// Сравнение с нулём
								codes.Add(new string[]{
									"mov "+IndexToReg(a)+", "+IndexToReg(1)+"\t\t; Перемещение из "+IndexToReg(a)+" в "+IndexToReg(1),
									"push "+IndexToReg(2)+"\t\t\t; Сохранение регистра "+IndexToReg(2)+" в стек",
									"nul "+IndexToReg(2)+"\t\t\t; Обнуление "+IndexToReg(2),
									"leq "+IndexToReg(1)+", "+IndexToReg(2)+"\t\t; Сравнение "+IndexToReg(1)+" с "+IndexToReg(1),
									"pop "+IndexToReg(2)+"\t\t\t; Возвращение значения "+IndexToReg(2)+" из стека"
								});
							} else {
								// Сравнение с регистром
								codes.Add(new string[]{
									"mov "+IndexToReg(a)+", "+IndexToReg(1)+"\t\t; Перемещение из "+IndexToReg(a)+" в "+IndexToReg(1),
									"geq "+IndexToReg(1)+", "+IndexToReg(b)+"\t\t; Сравнение "+IndexToReg(1)+" с "+IndexToReg(b),
								});
							}
						} else if (a == 0) {
							// Условный переход
							...
							if (b == c) {
								// Инкремент регистра
								codes.Add(new string[]{
									"inc "+IndexToReg(b)+"\t\t\t; Инкремент "+IndexToReg(b),
								});
							} else {
								// Чтение по регистру
								codes.Add(new string[]{
									"push "+IndexToReg(8)+"\t\t\t; Сохранение IN в стек",
									"peek "+IndexToReg(c)+"\t\t\t; Чтение из ячейки "+IndexToReg(c),
									"mov "+IndexToReg(8)+", "+IndexToReg(b)+"\t\t; Перемещение в регистр",
									"pop "+IndexToReg(8)+"\t\t\t; Чтение IN из стека",
								});
							}
						} else if (a == 9) {
							// Запись по адресу
							...
							if (b == c) {
								// Декремент регистра
								codes.Add(new string[]{
									"dec "+IndexToReg(b)+"\t\t\t; Декремент "+IndexToReg(b),
								});
							} else {
								// Чтение по регистру
								codes.Add(new string[]{
									"push "+IndexToReg(9)+"\t\t; Сохранение OUT в стек",
									"mov "+IndexToReg(b)+", "+IndexToReg(9)+"\t\t; Перемещение в регистр вывода",
									"poke "+IndexToReg(c)+"\t\t\t; Запись в память",
									"pop "+IndexToReg(9)+"\t\t\t; Чтение OUT из стека",
								});
							}
						}
					}
